...
struct thread_data{
     ...
     */
    //these values are required to be passed to the threadfunc
    //these values are to be passed to the relevant thread
    //using this structure and threadfunc we pass the values
    //for communication
    //stm -> structure member
    int stm_wait_to_obtain_ms;
    int stm_wait_to_release_ms;

    pthread_mutex_t *stm_mutex;
...
*
* NOTE: THIS IS A DIFFERENT LOCATION ::  start_thread_obtaining_mutex function should only start the thread and should not block
